...
<body>
	...
	<div class="wrap clr">
		...
		<div class="content">
			<div class="conTop">
				<span>您的当前位置：首页 >> 交流社区>> 社区论坛</span> <img src="images/tag04.png" />&nbsp; <strong>社区论坛</strong>Group Introduction
			</div>
			<div class="main">
				<div id="all">
					<!--查询模块-->
					<div class="subtitle">
						<div class="title">
							<c:forEach items="${ltbkList}" var="map" varStatus="mapStatus">
								<a href="javascript:void(0);" onclick="loadData('','','${map.ltbkName}');">${map.ltbkName
									}</a>
							</c:forEach>
						</div>
						<div class="editBlock1">
							<c:forEach items="${ltbqList}" var="map">
								<a href="queryByBq?ltbqName=${map.ltbqName}&ltbqId=${map.ltbqId}">${map.ltbqName}</a>
							</c:forEach>
						</div>
					</div>
					<div class="dGrid">
						<form action="BkBbsSave" method="post">
							<span> <textarea rows="5" cols="100" name="bbsNr" onkeyup="checkLength(this);">
									  发表您的论坛内容
									 </textarea> <input type="hidden" name="bbsBqid" id="bbsBqid" value="" />
							</span>
							<div class="data_btn">
								<span class="two"> / 200 &nbsp; <input name="button" type="submit" class="inputButton" value="发表" />
					...
					<table class="page">
						<tr>
							...
							<td style="width:265px;text-align: center;">
								当前显示 <span id="start">1</span> - <span id="end">10</span> 条记录 共 <span id="count">0</span> 条帖子记录
							</td>
						</tr>
					</table>
				</div>
