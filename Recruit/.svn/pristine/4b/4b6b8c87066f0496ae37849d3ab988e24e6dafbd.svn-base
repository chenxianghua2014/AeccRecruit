...
public class ZpglController extends BaseController {
	...
	List<Jtjlk> jtjlkss = CommonUtils.checkStuQuality(jtjlks);
	return jtjlkss;
    }
	...
	if ("未筛选,筛选协同".equals(filterStr)) { // 新的简历
	    fieldName.add("姓名");
	    fieldName.add("出生日期");
	    fieldName.add("身份证号");
	    fieldName.add("性别");
	...
	} else if ("通过,未测评,已测评,已安排面试".equals(filterStr)) { // 适合的简历
	    fieldName.add("姓名");
	    fieldName.add("出生日期");
	    fieldName.add("身份证号");
	    fieldName.add("性别");
	...
	} else if ("测评".equals(filterStr)) { // 已通知测评简历
	    fieldName.add("姓名");
	    fieldName.add("出生日期");
	    fieldName.add("身份证号");
	    fieldName.add("性别");
	...
	} else if ("一面进行中,二面进行中,终面进行中".equals(filterStr)) { // 已安排面试简历
	...
	} else if ("录用".equals(filterStr)) { // 已录用简历
	...
	} else if ("淘汰".equals(filterStr)) { // 已录用简历
	    System.out.println("录用");
	    fieldName.add("姓名");
	    fieldName.add("出生日期");
	    fieldName.add("身份证号");
	...
	} else if ("收藏".equals(filterStr)) { // 已录用简历
    ...
    /** 
    * @Title checkCpResult 
    * @Description 这种处理类都没注释！！！ffffff
    *              检测该生是否存在该岗位的测评成绩
    *              1.jtjlk中不存在该学生该岗位的记录，返回true
    *              2.若存在，且测评成绩为'未通知'或'未完成'，返回 true
    *              3.若存在该生该岗位成绩，做相应处理。。。。（待完善）
    * @param type 安排测试的类别： 管理岗、技术岗
    * @param id jtjlk的id
    * @param session
    * @return boolean     
    */
	...
	if (cr != null) {
		...
		try {
		    ...
		    String strContent = "姓名：" + jtjlk.getJtjlkName() + "，" + "学校：" + jtjlk.getJtjlkByxx() + "，" + "专业：" + jtjlk.getJtjlkZy() + "，" + "测评成绩：" + cr.getJtjlkCpcj()
			    + "。请登录后台管理系统查看应聘者性格测试报告。";
		    Xxtz xxtz = new Xxtz();
    ...
    */
	...
	for (int i = 0; i < strJtjlkIds.length; i++) {
		...
		XmlEntity xmlentity = new XmlEntity();
		SendMessage s = new SendMessage();
		String xxtzMessageContent = "面试通知——面试官:" + txl.getTxlName() + ",此次登录系统帐号信息如下:帐号:" + userinfo.getUserIdcard() + "密码:" + userinfo.getUserPassword()
			+ "面试完毕后请登录系统填写面试结果.【中国航天科工培训中心】";
		xml = s.SendMessage("5726", "xpt20111", "htpxzx123456", txl.getTxlPhone(), xxtzMessageContent, "").toString();
		// System.out.println(xml);
		xmlentity.setReturnstatus("returnstatus");
		xmlentity.setMessage("message");
	...
	return CommonUtils.checkStuQuality(jtjlks);
    }
	...
	for (Jtjlk jtjlk : jtjlks) {
	    Zzjg z = zzjgService.selectByPrimaryKey(jtjlk.getZzjgId());
	    jtjlk.setZzjgName(z.getZzjgDwmc());
	    z = null;
	}
