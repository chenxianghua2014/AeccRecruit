				...
				success : function(zpxxs) {
					...
					for (var i = 0; i <= zpxxs.length - 1; i++) {
						...
						if (zpxxs[i].zpxwTop == 0) {
							strHtml += "<input name='button' type='button' onclick=\"setTop('"
									+ zpxxs[i].zpxwId + "',1, this.value);\") ";
							strHtml += "class='inputButton' value='取消置顶' />";
						} else {
							strHtml += "<input name='button' type='button' onclick=\"setTop('"
									+ zpxxs[i].zpxwId + "',0, this.value);\") ";
							strHtml += "class='inputButton' value='置顶' />";
						}
						strHtml += "</td>";

						strHtml += "<td class='alignCenter'><input name='button'";
						strHtml += "		type='button' onclick=\"window.location.href='ZpxwContent?id="
								+ zpxxs[i].zpxwId + "'\")";
						strHtml += "	 	class='inputButton' value='查看' />";
						strHtml += "</td>";

						strHtml += "<td class='alignCenter'><input name='button'";
						strHtml += "		type='button' onclick=\"ZpxwDel('"
								+ zpxxs[i].zpxwId + "');\"";
...
function setTop(_id, _status, ve) {
	var html = "<div style=\"padding-top: 8px;\">&nbsp;有效时间至：<input type='text' id='rtime' name='rtime' class=\"Wdate\" onFocus=\"WdatePicker({isShowClear:false,dateFmt: 'yyyy-MM-dd HH:mm:ss', readOnly:true})\" /></div>";
	if (ve == "取消置顶")
		html = "您确认要取消置顶？";
	var submit = function(v, h, f) {
		if (f.rtime == '') {
			$.jBox.tip("有效时间至 不能为空。", 'error', {
				focusId : "rtime"
			});
			return false;
		} else {
