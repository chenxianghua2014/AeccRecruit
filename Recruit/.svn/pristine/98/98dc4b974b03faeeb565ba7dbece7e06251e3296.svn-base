			...
			.ajax({
				...
				success : function(result) {
					for (var i = 0; i <= result.model.list.length - 1; i++) {
						...
						/*薪资范围  10~20*/
						strHtml += "<td>"
							+ (result.model.list[i].payLow == null ? ""
									: result.model.list[i].payLow)
							+"~"
							+ (result.model.list[i].payHigh == null ? ""
									: result.model.list[i].payHigh)
							+ "</td>";
						/*支持政策*/
						strHtml += "<td><div class=\"tt\" onmouseover=\"mOver(this)\" onmouseout=\"mOut(this)\">"
							+ result.model.list[i].positionSpepol
							+ "</div></td>";
						strHtml += "<td>"
...
var promptUni = function() {
	var strHtml = "<div class='editBlock'><table>"
			...
			+"</tr>" */
			+ "</table></div>";
	$.jBox.open(strHtml, '优才计划模块温馨提示', 350, 350, {
		buttons : {
			'关闭' : true
		}
	});
