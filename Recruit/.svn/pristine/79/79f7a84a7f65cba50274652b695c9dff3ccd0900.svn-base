	...
	<div class="wrap clr">
		<div class="menu">
			...
			<ul>
				...
				<li><a href="MyAllApplication" style="color:#0046ae;">网上测评预约</a></li>
			</ul>
		</div>
		<div class="content">
			<div class="conTop">
				<span>您的当前位置：首页 >> 校园招聘 >> 我的求职 >> 网上测评预约</span> <img src="images/tag04.png" />&nbsp; <strong>网上测评预约</strong>
			</div>
			<div class="result">
				<div class="resultTitle">
					<img src="images/tag05.png" />测评预约 
				</div>
				<div class="subtitle_pl">
					<c:choose>
						<c:when test="${Applications == null || fn:length(Applications) == 0}">
							<p style="text-indent: 5em;">您暂无职位申请记录。</p>
						</c:when>
						<c:otherwise>
							<table width="800px">
								<thead>
								...
								<tbody>
									<c:forEach items="${ Applications}" var="map" varStatus="status">
										<tr>
											<td>${ map.sqdw}</td>
											<td>
												<a href="javascript:popbox('ZwglView?id=${map.pid}');">${ map.sqgw}</a>
											</td>
											<td>${ map.jlsx}</td>
											<td>${ map.apcpgwlb}</td>
											<c:choose>
												...
												<c:otherwise>
													<c:choose>
														<c:when test="${ map.bmglid eq null}">
															<td>
																<a href="Xzcc?id=${ map.jtjlkid}">网上测评预约</a>
															</td>
															<td>--</td>
														</c:when>
														<c:otherwise>
															...
															<td>
																<a href="PrintTestCard?id=${ map.bmglid}">打印准考证</a>
															</td>
														</c:otherwise>
													</c:choose>
												</c:otherwise>
				...
				<div class="resultTitle">
					<img src="images/tag05.png" />优才计划测评预约
				</div>
				<div class="subtitle_pl">
					<c:choose>
						<c:when test="${UniApplications == null || fn:length(UniApplications) == 0}">
							<p style="text-indent: 5em;">您暂无优才职位申请记录。</p>
						</c:when>
						<c:otherwise>
							<table width="800px">
								<thead>
								...
								<tbody>
									<c:forEach items="${UniApplications}" var="map" varStatus="status">
										<tr>
											<td>${ map.sqdw}</td>
											<td>
												<a href="javascript:popbox('ZwglView?id=${map.pid}');">${ map.sqgw}</a>
