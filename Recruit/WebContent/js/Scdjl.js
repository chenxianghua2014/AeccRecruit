			...
			.ajax({
				...
				success : function(jtjlks) {
					for (var i = 0; i <= jtjlks.length - 1; i++) {
						 ...
						 * strHtml += "<td>" + jtjlks[i].jtjlkUniPositionZw + "</td>";
						 */
						...
						/* 优才详情 */
						strHtml += "<td class='alignCenter'><input name='button'";
						strHtml += "		type='button' onclick=\"javascript:parent.redirect('UniApplicationHr?userId="
								+ jtjlks[i].jtjlkUserid
								+ "');\" class='inputButton' value='查看' /></td>";
...
function _Do(jtjlkId, jtjlkUserid, jtjlkCpcj, jtjlkMszt, jtjlkCpjg, jtjlkName,
		jtjlkPositionId,jtjlkUniPositionId, jtjlkZw) {
	$("#divControllBatch").hide();
	$('#divControll').show();
	//jtjlkPositionId存在或者jtjlkUniPositionId存在
	if ((jtjlkPositionId != "" && jtjlkPositionId != "无" && jtjlkPositionId != null) ||
		(jtjlkUniPositionId != "" && jtjlkUniPositionId != "无" && jtjlkUniPositionId != null)) {
		$('#zw').html(jtjlkZw);
		$("#zw").show();
		$("#tdApzw").hide();
	} else {
	...
	$('#name').html(jtjlkName);
	if(jtjlkPositionId == "无" || jtjlkPositionId == null){
		$('#jtjlkPositionId').val("");
	}else{
		$('#jtjlkPositionId').val(jtjlkPositionId);
	}
	if(jtjlkUniPositionId == "无" || jtjlkUniPositionId == null){
		$('#jtjlkUniPositionId').val("");
	}else{
		$('#jtjlkUniPositionId').val(jtjlkUniPositionId);
	}
...
function check(type) {
	...
	var uname = new Array();
	if (type != "batch") {
		if ($("#jtjlkPositionId").val() == "" && $("#jtjlkUniPositionId").val() == "") {
			alert("请先对收藏的简历安排职位!");
			return false;
		}
		$("#jtjlkId").val($("#Id").val());
