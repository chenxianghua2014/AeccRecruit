...
<body>
	...
	<div class="wrap clr">
		...
		<div class="content">
			<div class="conTop">
				<span>您的当前位置：首页 >> 校园招聘 >> 我的求职 >> 优才计划申请</span> <img src="images/tag04.png" />&nbsp; <strong>我的求职</strong>My Recruit
			</div>
			<div class="result">
				<div class="Tabbg">
					...
					<a class="spe Tabactive" href="MyUniApplication">优才计划申请</a>
												...
												<tr>
														...
														<c:otherwise>
															<c:choose>
																...
																<c:otherwise>
																	...
																	<td><a href="PrintTestCard?id=${ map.bmglid}">打印准考证</a></td>
																</c:otherwise>
															</c:choose>
														</c:otherwise>
													</c:choose>
													<c:choose>
														<c:when test="${map.cpcj eq '未通知'}">
															<td><a href="javascript:void(0)" onclick="Revoke('${ map.jtjlkid}','${ map.apcpgwlb}')">撤销申请</a></td>
														</c:when>
	...
	function Revoke(id, type) {
		if (confirm("您确认要撤销对该单位的职位申请吗 ？您已约考，本次申请撤销将取消您对" + type
			...
			if (confirm("您确认要撤销对该单位的职位申请吗 ？您已约考，本次申请撤销将取消您对"
					+ type
					+ "岗位的约考记录，若还有其他单位通知您测评相同岗位，请您务必再次约考并打印准考证（原准考证做废），以免影响正常测评！"))
				window.location.href = "UniRevoke?id=" + id;
	}
</script>
</html>