...
<body>
	...
	<div class="wrap clr">
		...
		<div class="content">
			<div class="conTop">
				<span>您的当前位置：首页 >> 校园招聘 >> 我的求职 >> 消息中心</span> <img src="images/tag04.png" />&nbsp; <strong>我的求职</strong>My Recruit
			</div>
			<div class="result">
				<div class="Tabbg">
					...
					<a class="spe" href="MyUniCollection">优才计划收藏</a>
					<a class="Tabactive" href="Message">消息中心</a>
				</div>
					...
					<c:choose>
						...
						<c:otherwise>
							<c:choose>
								...
								<c:otherwise>
									<c:forEach items="${ mv.model['xxtzs']}" var="map" varStatus="status">
										<li><c:if test="${ map.xxtzIsread eq 1}">
												<strong><a style="text-decoration:none;color: black;" href="MessageDetails?id=${ map.xxtzId}">${
															map.xxtzType}</a>
													<span><fmt:formatDate value="${ map.xxtzAddtime}" type="date" /> </span> </strong>
											</c:if> <c:if test="${ map.xxtzIsread eq 0}">
												<a style="text-decoration:none;color: black;" href="MessageDetails?id=${ map.xxtzId}">${
														map.xxtzType}</a>
												<span><fmt:formatDate value="${ map.xxtzAddtime}" type="date" /> </span>
											</c:if></li>
									</c:forEach>
								</c:otherwise>
							</c:choose>
