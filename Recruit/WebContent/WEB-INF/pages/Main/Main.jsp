...
<body>
	...
	<div class="wrap clr">
		...
		<div class="information">
			<div class="infMore">
				<span style="cursor:pointer;" onclick="window.location.href='InformationCenter'">&gt;&gt;更多</span>
			</div>
			...
			<div class="infList">
				<ul>
					<c:forEach items="${ mv.model['zpxw']}" var="map" varStatus="status">
						<c:choose>
							<c:when test="${ map.zzjgId == 'test001'}">
								<li><a style="color: red;" href="ViewNews?id=${map.zpxwId}">${map.zpxwTitle}</a><span style="color: red;">${map.zpxwFbsj}</span></li>
							</c:when>
							<c:otherwise>
								<li><a href="ViewNews?id=${map.zpxwId}">${map.zpxwTitle}</a><span>${map.zpxwFbsj}</span></li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
				</ul>
		...
		<div class="LeftBtn">
			<a href="javascript:void(0);" onclick="javascript:window.location='MyRecruit'">我的求职</a>
			<a href="javascript:void(0);" onclick="javascript:window.location='MyAllApplication'">网上测评预约</a>
			<!-- <a href="javascript:void(0);" onclick="javascript:window.location='MyUniApplication'">优才职位测评预约</a> -->
			<a href="javascript:void(0);" onclick="javascript:window.location='SearchUniPosition'">优才计划</a>
		</div>
	</div>
	<div class="wrap clr">
		<div class="companyname" style="margin-right:20px;">
			...
			<div class="comCon">
				<ul>
					<c:forEach items="${ mv.model['zzjg']}" var="map" varStatus="status">
						<li class="menu2" onmouseover="this.className='menu1'" onmouseout="this.className='menu2'"><a href="javascript:void(0);"
								onmouseover="this.className='Af'" onmouseout="this.className='AfNone'" class="Af"
								onclick="window.location.href='InstituteOf?id=${map.zzjgId}'">${map.zzjgDwmc}</a>
							<div class="list">
								<ul style="width:412px;">
									<c:forEach items="${ mv.model['zzjg3']}" var="zzjg3" varStatus="status">
										<c:choose>
											<c:when test="${map.zzjgId eq zzjg3.zzjgSjdw}">
												<li><a href="javascript:void(0);" onclick="window.location.href='InstituteOf?id=${zzjg3.zzjgId}'">${zzjg3.zzjgDwmc}</a></li>
											</c:when>
										</c:choose>
									</c:forEach>
								</ul>
		...
		<div class="companyname">
			...
			<div class="comCon">
				<ul>
					<c:forEach items="${ mv.model['zylb']}" var="map" varStatus="status">
						<li class="menu2" onmouseover="this.className='menu1'" onmouseout="this.className='menu2'"><a href="javascript:void(0);"
								onmouseover="this.className='Af'" onmouseout="this.className='AfNone'" class="Af"
							...
							<div class="list">
								<ul style="width:412px;">
									<c:forEach items="${ map.zpzys}" var="mapChi" varStatus="status">
										<li><a href="javascript:void(0);" onclick="window.location.href='SearchPosition?zpzy='+escape('${ mapChi.zpzyName}')">${
												mapChi.zpzyName}</a>
										</li>
									</c:forEach>
								</ul>
			...
			<div class="search">
				<table width="100%">
					<tr>
						<td>招聘单位</td>
						<td>
							<input type="text" style="width: 146px;" id="zpdw" name="zpdw" />
						</td>
						<td>招聘专业</td>
						<td>
							<input type="text" style="width: 146px;" id="zpzy" name="zpzy" />
						</td>
						<td>职位类别</td>
						<td>
							<select style="width: 150px;" id="zwlb" name="zwlb">
								<option></option>
					...
					<tr>
						<td>工作地点</td>
						<td>
							<select style="width: 150px;" id="gzdd" name="gzdd">
								<option value=""></option>
						...
						<td>学历要求</td>
						<td>
							<select style="width: 150px;" id="xlyq" name="xlyq">
								<option value=""></option>
						...
						<td>发布时间</td>
						<td>
							<select name="dt" id="dt" style="width: 150px;">
								<option value=""></option>
					...
					<tr>
						<td colspan="4"></td>
						<td class="jobsea">
							<a href="javascript:void(0);" onclick="seachPosition();">
								<span>招聘职位</span>
							</a>
						</td>
						<td class="jobsea">
							<a href="javascript:void(0);" onclick="seachUniPosition();">
								<span>优才计划</span>
							</a>
						</td>
					</tr>
	...
	<c:choose>
		<c:when test="${userLoginSession eq null}">
			<script type="text/javascript">
				$.jBox.open("iframe:LoginFrm", "用户登录", 280, 230, {
					buttons : {}
				});
			</script>
		</c:when>
